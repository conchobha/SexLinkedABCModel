...
RunModel <- function(dataname,dn,sn,fn,num,burns,group,form)
{
  ...
  res=list("model"=md,"dn"=dn,"sn"=sn, "fn"=fn,"Iteration"=num 
           ,'testX' = X_test,'testY' = Y_test
           #,"validation.id"=validation.id, "test.id"=test.id,"train.id"=train.id #-> Removed for now, will return later 
  )
  
  
  outfile <- paste0("/N/u/conlcorn/BigRed200/SexLinkedProject/output/model_outputs/Dimensonality_testing_2/",dn,"/Rep_",num)
...
clusterExport(cl, varlist = c("RunModel", "dn", "sn", "fn", "burns", "rep", "grouplist", "datametriclist"))
...
clusterEvalQ(cl, {
  ...
  library(readr)
  library(dplyr)
  message("Libraries loaded successfully on worker")
})

# Debug cluster setup
clusterEvalQ(cl, Sys.info()[['nodename']])

# Parallel execution with additional logging
tryCatch({
  results <- foreach(d = dn, .combine = 'list', .packages = c('MASS', 'foreach', 'doParallel', 'readr', 'dplyr'), .inorder = FALSE) %:%
    foreach(g = datametriclist) %:%
    foreach(group = grouplist) %:%
    foreach(r = rep) %do% {
      tryCatch({
        message(paste("Running model with: group =", group, "d =", d, "sn =", sn))
        RunModel(g, d, sn, fn, r, burns, group, 'mean')
        message("Finished model for: group =", group, "d =", d)
      }, error = function(e) {
        message(paste0("Error in worker: ", e))
        NULL
      })
    }
  message("Execution completed.")
}, error = function(e) {
  message(paste0("An error occurred: ", e))
})
